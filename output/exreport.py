...
def export(tr,name):
    ...
    try:
        ...
        logging.error("Error in exporting raw data")
        print sys.exc_info()

    try:
        export_fig(tr,name)
    except:
        logging.error("Error in creating plots")
        print sys.exc_info()
...
def export_fig(tr,base_filename):
    ...
    if "population.proportions" in tr.traces[0]:
        logging.info("Generating population.proportions plots...")
    ...
    if "global_proportions" in tr.traces[0]:
        logging.info("Generating global_proportions plots...")
